...
MDNode *DebugLoc::getScope(const LLVMContext &Ctx) const {
  ...
  if (ScopeIdx > 0) {
    sys::CondScopedLock locked(Ctx.pImpl->mutexScopeRecords);
  ...
  sys::CondScopedLock locked(Ctx.pImpl->mutexScopeInlinedAtRecords);
...
int LLVMContextImpl::getOrAddScopeRecordIdxEntry(MDNode *Scope,
                                                 int ExistingIdx) {
  sys::CondScopedLock locked(mutexScopeRecords);
...
int LLVMContextImpl::getOrAddScopeInlinedAtIdxEntry(MDNode *Scope, MDNode *IA,
                                                    int ExistingIdx) {
  sys::CondScopedLock locked(mutexScopeInlinedAtRecords);
...
void DebugRecVH::deleted() {
  ...
  if (Idx > 0) {
    {
    sys::CondScopedLock locked(Ctx->mutexScopeRecords);
  ...
  {
  sys::CondScopedLock locked(Ctx->mutexScopeInlinedAtRecords);
